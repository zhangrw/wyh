...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		if(ts.p.autowidth===true) {
			...
			var eg1 = $("<div class='ui-jqgrid ui-widget ui-widget-content ui-corner-all'></div>");
			...
			for (i = 0; i < cml; i++) {
				...
				if (iCol >= 0) {
					...
					$colHeader = $('<th>').attr({role: "columnheader"})
						...
						.css({'height':'22px', 'border-top': '0px none','text-align':'center'})//'text-align':'center' add by John
						.html(titleText);
	...
	setFrozenColumns : function () {
		return this.each(function() {
			...
			if( maxfrozen>=0 && frozen) {
				...
				hth = $(".ui-jqgrid-htable","#gview_"+$.jgrid.jqID($t.p.id)).height()+1;//add +1 by John
				//headers
				if($t.p.toppager) {
